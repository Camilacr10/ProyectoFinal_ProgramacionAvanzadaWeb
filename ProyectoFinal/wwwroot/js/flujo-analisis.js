    ...
    const Analisis = {
        ...
        inicializarTabla() {
            this.tabla = $('#tablaAnalisis').DataTable({
                ajax: { url: '/FlujoSolicitudes/ObtenerAnalisis', type: 'GET', dataSrc: 'data' },
                columns: [
        ...
        registrarEventos() {
            $('#tablaAnalisis').on('click', '.enviar', (e) => this.cambiar($(e.currentTarget).data('id'), 'EnviadoAprobacion'));
            $('#tablaAnalisis').on('click', '.devolver', (e) => this.cambiar($(e.currentTarget).data('id'), 'Devolucion'));
        },
